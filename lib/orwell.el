...
Subject: orwell.el: swap C-h & DEL without affecting prefixes
Newsgroups: gnu.emacs.sources
Organization: Eskimo North (206) For-Ever
Message-ID: <01bcbe69$e6f216c0$0cfb47c1@im2u>
...
;; I'm not the first human to ever want this, & let me tell you a story:
when
;; I first wanted to learn Emacs, the only access I had to it was on a Unix
;; shell on a dial-up ISP that I reached via an 80286 emulating a VT100, &
I
;; had absolutely no documentation whatsoever; the ISP's staff were having
a
;; bad-hair year, the installation didn't have a man page, & I didn't know
;; about Info yet; but I remembered being told that C-h was help.  So I
tried
...
;; the only reason I didn't need to generate a SIGHUP by hanging up the
modem
;; just to get out of Emacs, was that I knew about C-x C-c from some guy
who
;; used the Epsilon editor.  It was SEVERAL DAYS before I learned how to
run
;; M-x help-on-help.  And that of course sucks, so I didn't learn much
Emacs
;; until an upgrade by a new sysop eliminated this "improved" keymapping.
;; NEVER DO THIS AT THE SITE LEVEL!  Wanting to swap <Ctrl-h> and
<Backspace>
...
;; nontrivial, and that's even when Emacs itself is working; the Windows
port
...
;; A 26 Mar 97 Erik Naggum Usenet post suggests the character syntax has
been
...
;; the second, since that'd just make <Backspace> & <Ctrl-h> synonyms for
the
;; same command.
...
;; If you experiment with keyboard-translate on an ISP Unix shell, watch
out
...
;; <DownAlt><kp-2><kp-5><kp-5><UpAlt>.  If you're using a
keyboard-translate
...
;; response to a (describe-key-briefly) prompt you'll see that the mapping
of
...
;; To get the effect that C-h C-h has in Emacs, we need to insert [H-h]
into
...
;; putting the new event into that list causes the message printed to be
more
...
;; whatever is mapped to DEL.  I can't usefully put a reciprocal mapping
into
...
;; the Hyper key, so C-x @ h h should be equivalent to H-h.  Well, I've
found
...
;; Unfortunately, that's not good enough for isearch.el v19.34.1, which
needs
...
;; isearch will delete the char under point *in the buffer* you're
searching;
...
;; DEL at all.  It's necessary to hack the map in a hook.  Since isearch
has
...
;; <sigh> Looking at the latest code, I see that RMS turned off the
original
...
;; straightforward to just terminate the isearch first with C-m, & most
users
...
;; want global help instead.  If none of that ever happens you can still
live
...
;; Any other obscure modes that need similar hook hacks can get them here:

;? Is this one (legacy from [255]) still necessary?  Probably not.
;? (add-hook 'sgml-mode-hook '(lambda ()
;?   (define-key sgml-mode-map [H-h] 'help-command)))

;; The next problem is that on a remote 7-bit terminal where the only way
to
...
;; delete are the same thing; the manual says C-M-DEL, but all the
functions
...
;; between what <Backspace> and <Ctrl-Backspace> transmit; they're both
DEL.
...
;; THE RESULT SO FAR: At this point the effect has been to make DEL the
help
...
;; involving DEL, but I haven't thought of one.  The best I can do is make
it
;; possible to toggle whether the translation of DEL actually occurs;
toggle
...
(defun del ()
  ...
  (interactive)    
  (cond ((not (boundp 'translation-of-127-is-toggled-off))
        ...
        (t                     
         (define-key key-translation-map [127] [H-h])
         (message "<Backspace> is now H-h, the help char; use C-h to
erase.")
         (makunbound 'translation-of-127-is-toggled-off))))
...
;; improvements that it makes toggleable.  It could even be a minor mode,
but
;; I think that most of the time having it take space on the modeline would
;; be undesirable, since most of the time its users will have it always on.
;; (For an example of the opposite situation, I think toggle-rot13-mode
errs
...
;; which button to press to get help (NOT "[H-h]", which would be
meaningless
...
;; I'd prefer that that message not clutter up *Messages*; is that
possible?
...
;; I want the <F1> key on a PC, when talking to an ISP Unix shell by way of
a
;; comm program running a VT100 emulation, to have the standard meaning it
has
;; in the PC world: pressing the key gets help.  19.34.1 already so maps
[f1].
;; However, via a VT100 emulation, an <F1> is transmitted to Emacs not as
[f1]
;; but as [kp-f1].  Because of the importance of help, I think it's a bug
in
...
;; Note that consing onto the front would be bad, because it changes the
car.
