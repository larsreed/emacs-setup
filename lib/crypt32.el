...
Here is `crypt++.el' with changes I made to allow
this package to work on Win32.

The crucial changes are at the beginnings of
`crypt-find-file-hook()' and `crypt-write-file-hook()'.

This package will allow you to transparently visit `.Z',
`.gz', `.tar', `.tar.gz' and DOS files.  (However, on
occasion, certain `.tar.gz' files lock up emacs...don't
know why, but killing `gzip' frees up emacs again.)

Sorry for the long post; if anyone has any trouble,
please let me know.
