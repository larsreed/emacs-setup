...
;; Contents: Merge of help.el from 20.3.1 + personal additions (lre)
...
(define-key help-map "Pl" 'locate-library)
(define-key help-map "Pf" 'find-function)
(define-key help-map "Pk" 'find-function-on-key)
...
(make-help-screen help-for-help
  "[?CFIKMSVabcdefiklmnpqstvwy] 4i P[lfk] A[Aauv] C-[cdfiklnpvw] M-w; ? for help:"
  "You have typed %THIS-KEY%, the help character.  Type a Help option:
\(Use SPC or DEL to scroll through this text.  Type \\<help-map>\\[help-quit] to exit the Help command.)

4i info-other-windows.  Start Info in another window.
a  apropos-command.  Give a substring, and see a list of commands
	(functions interactively callable) that contain
	that substring.
AA  apropos.  Give a substring, and see a list of commands and variables
	that contain that substring.  With prefix: show key bindings
	and unbound symbols as well.
Aa like a.
Ad  apropos-documentation.  Show symbols whose documentation contain matches
	for REGEXP.  With prefix:  also use documentation that is not stored
	in the documentation file and show key bindings.
Au  apropos-value.  Show all symbols whose value\'s printed image matches
	REGEXP.  With prefix: also look at the function and at the names and
	values of properties.
Av  apropos-variable.  Show user variables that match REGEXP.
	With prefix: also show normal variables.
...
d  list-load-path-shadows.  List all duplicate files in load-path.
C-d Display Emacs ordering information.
e  describe-face.  Enter a face name to view its description.
...
n  view-emacs-news.  Shows emacs news file.  (Prefix for OO...NEWS)
C-n Display news of recent Emacs changes.
p  finder-by-keyword. Find packages matching a given topic keyword.
Pl  locate-library.  Give the name of a library, to find where it is
	located.
Pf  find-function.  Open library containing given function.
Pk  find-function-on-key.  Open library containing function for a given
        key-stroke.
...
M-w bill-help-on-topic.  Show Windows HLP...
V  view-sun4at-keys.
	View the file containing information about special sun4 at101 keys
y  describe-syntax.  Describe the syntax specifications in the syntax table.
"
  help-map)
...
Returns the documentation as a string, also."
  ...
  (if (symbolp variable)
	...
	(with-output-to-temp-buffer "*Help*"
	  ...
	  ;; has been loaded.
	  (if (or (get variable 'standard-value)
