...
;; Version: $Id: xpath.el,v 1.8 2001/12/16 23:28:11 alex Exp $
...
This returns a list of nodes."
  (xpath-resolve-steps node (xpath-steps xpath)))
...
(eval-when-compile
  (when (file-readable-p "sample.xml")
	...
	(assert (equal (mapcar 'dom-node-name (xpath-ancestor-axis title))
		       '(bookbiblio bookinfo book)))
	(assert (equal (mapcar 'dom-node-name (xpath-ancestor-or-self-axis title))
		       '(title bookbiblio bookinfo book)))
	(assert (equal (mapcar 'dom-node-name
			       (xpath-attribute-axis (dom-document-element doc)))
		       '(id)))
	(assert (equal (mapcar 'dom-node-name
			       (xpath-child-axis (dom-document-element doc)))
		       '(bookinfo chapter)))
	(assert (equal (mapcar 'dom-node-name
			       (xpath-descendant-axis
				(dom-element-last-child (dom-document-element doc))))
	...
	(assert (equal (mapcar 'dom-node-name
				...
				(dom-element-first-child (dom-document-element doc))))
		       '(chapter title \#text para \#text)))
	(assert (equal (mapcar 'dom-node-name (xpath-following-axis title))
		       '(edition \#text authorgroup author firstname \#text
				 surname \#text chapter title \#text para \#text)))
	(assert (equal (mapcar 'dom-node-name (xpath-following-sibling-axis title))
		       '(edition authorgroup)))
	(assert (equal (mapcar 'dom-node-name (xpath-parent-axis title))
		       '(bookbiblio)))
	(assert (equal (mapcar 'dom-node-name
			       (xpath-preceding-axis
				(dom-node-last-child (dom-document-element doc))))
		       '(\#text surname \#text firstname author authorgroup \#text
			 edition \#text title bookbiblio bookinfo)))
	(assert (equal (mapcar 'dom-node-name
				...
				(dom-node-last-child; authorgroup
				 (dom-node-first-child; bookbiblio
				  (dom-node-first-child; bookinfo
				   (dom-document-element doc)))))); book
		       '(edition title)))
	(assert (equal (xpath-self-axis title) (list title))))

      (let* ((doc (dom-make-document-from-xml data))
	     (node-list (dom-document-get-elements-by-tag-name doc '*)))
	(assert (equal (mapcar 'dom-node-name (xpath-name-filter node-list 'title))
		       '(title title)))
	(assert (equal (mapcar 'dom-node-value (xpath-text-filter node-list))
		       '("My own book!" "First" "John" "Wiegley" "A very small chapter"
	...
	(assert (equal (mapcar 'dom-node-name
		       ...
		       '(bookinfo chapter)))
	(assert (equal (mapcar 'dom-node-name
			       (xpath-resolve-axis (dom-node-child-nodes root)
						   'xpath-child-axis))
		       '(bookbiblio title para)))
	(assert (equal (mapcar 'dom-node-text-content
			       (xpath-resolve root "descendant::title"))
		       '("My own book!" 
			 "A very small chapter")))
	(assert (equal (mapcar 'dom-node-text-content
			       (xpath-resolve root "descendant::chapter/child::title"))
		       '("A very small chapter"))))

      (assert (xpath-equal '(yes) t 5))
      (assert (not (xpath-equal '(yes) nil 5)))
      (assert (xpath-equal '(yes) 5.0 5))
      (assert (not (xpath-equal '(yes) 4 5)))
      (assert (xpath-equal '(yes) "5.0" 5))
      (assert (xpath-equal '(yes) '(+ 3 2) 5))
      (assert (equal (xpath-resolve-args '(1 (= 1 1) 3))
		     '(1 t 3)))
      (assert (equal (xpath-resolve-args '(4 (+ 3 2) 6))
		     '(4 5 6)))
      (assert (xpath-equal '(yes) '(1 2 3) 3))
      (assert (not (xpath-equal '(yes) '(1 2 3) 4)))
      (assert (xpath-equal '(yes) 3 '(1 2 3)))
      (assert (not (xpath-equal '(yes) 4 '(1 2 3))))
      (assert (xpath-equal '(yes) '(1 2 3) '(3 4 5)))
      (assert (not (xpath-equal '(yes) '(1 2 3) '(4 5 6))))

      (let* ((doc (dom-make-document-from-xml data))
	     (root (dom-document-element doc)))
	(assert (equal (mapcar 'dom-node-name (xpath-resolve root "child::*[position()=1]"))
		       '(bookinfo)))
	(assert (equal (mapcar 'dom-node-name (xpath-resolve root "child::*[position()=2]"))
		       '(chapter)))
	(assert (null (xpath-resolve root "child::*[attribute::id=\"compiler\"]"))))

      (let* ((doc (dom-make-document-from-xml data))
	     (root (dom-document-element doc)))
	(assert (equal (mapcar 'dom-node-name (xpath-resolve root "self::*[attribute::id=\"compiler\"]"))
		       '(book))))
      )))
