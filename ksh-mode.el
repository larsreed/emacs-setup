...
;; $Source: /usr/CVSroot/XEmacs/xemacs-packages/prog/prog-modes/ksh-mode.el,v $ --
;;
;; LCD Archive Entry:
...
;"alias" "autoload" "bg" "break" "continue" "cd" "exit" "echo" "fc" "fg" "getopts" "jobs"
;"kill" "let" "newgrp" "print" "pwd" "read" "readonly" "return" "set" "shift"
;"test" "times" "trap" "typeset" "ulimit" "umask" "unalias" "unset" "wait" "whence"))

;; LRE - add autoload
(setq ksh-keywords "a\\(lias\\|utoload\\)\\|b\\(g\\|reak\\)\\|c\\(ase\\|d\\|ontinue\\)\\|do\\(\\|ne\\)\\|e\\(cho\\|l\\(if\\|se\\)\\|sac\\|x\\(it\\|port\\)\\)\\|f\\([cgi]\\|or\\|unction\\)\\|getopts\\|i[fn]\\|jobs\\|kill\\|let\\|newgrp\\|p\\(rint\\|wd\\)\\|re\\(ad\\(\\|only\\)\\|turn\\)\\|s\\(e\\(lect\\|t\\)\\|hift\\)\\|t\\(est\\|hen\\|imes?\\|rap\\|ypeset\\)\\|u\\(limit\\|mask\\|n\\(alias\\|set\\|til\\)\\)\\|w\\(ait\\|h\\(ence\\|ile\\)\\)")
...
(defvar ksh-font-lock-keywords (purecopy
      (list
       ...
       (cons (concat
              ...
              "\\)\\>")
	     1)
       ;; Fontify function names -- LRE added num/. as 1. char
       '("\\<function[ \t]+\\([^(; \t]+\\)" 1 font-lock-function-name-face)
       '("\\(^[ \t]*[A-Za-z_0-9.]+[ \t]*()\\)" 1 font-lock-function-name-face)
       ;; Fontify exports
       '("\\<export[ \t]+\\(-[a-z][ \t]+\\)?\\<\\([A-Za-z_][A-Za-z0-9_]*\\)\\>" 2 font-lock-variable-name-face)
       ;; Fontify aliases -- LRE - addded "." + first char
       '("\\<alias[ \t]+\\(-[a-z][ \t]+\\)?\\<\\([A-Za-z0-9_.]+\\)\\>" 2 font-lock-type-face)
       ;; Fontify autoloads -- LRE added
       '("\\<autoload[ \t]+\\<\\([A-Za-z0-9_.]+\\)\\>" 1 font-lock-type-face)
...
;; LRE:
(defvar ksh-imenu-generic-expression
  '((nil "^\\s-*function\\s-+\\([A-Za-z_0-9.]+\\)" 1)
    (nil "^\\s-*\\([A-Za-z_][A-Za-z_0-9]*\\s-*()\\)" 1)
    ("*variables*"
     "^\\s-*\\(typeset\\||integer\\|readonly\\)\\s-+\\(-[A-Za-z][a-zA-Z0-9]*\\s-+\\)?\\([A-Za-z_0-9.]+\\)"
     3)
...
Installation:
 ...
 (setq ksh-mode-hook
  ...
  (setq imenu-generic-expression ksh-imenu-generic-expression)
  ;;
