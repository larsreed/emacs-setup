...
;; Keywords:     mouse
;; Version:	 1.0
;; Release-date: 1-Mar-1998
;; Location:     http://www.lysator.liu.se/~tab/artist/
;; Id:           $Id: artist.el,v 1.26 1998/03/01 14:41:30 tab Exp $
...
;; 
;; * Shapes: The following shapes are implemented:
;;     lines,	straight-lines,
;;     rectangles,	squares,
;;     poly-lines,	straight poly-lines,
;;     ellipses,	circles
;;   Straight lines are lines that go horinzontally, vertically or
;;   diagonally. Plain lines go in any direction. The shapes in the right
;;   column are accessed by holding down the shift key while drawing.
...
;; Artist is developed and tested with GNU Emacs 19.34. It seams to
...
;; 1.0-beta5    22-Feb-1998
;; Minor bugfixes

;; 1.0-beta4    21-Feb-1998
;; Added ellipse and circle drawing.
;; Added flood-filling.

;; 1.0-beta3    10-Feb-1998
;; A new bug-submit-feature (brought to you by the reporter package).
;; Also, Trimming of line-endings is now both in the popup-menu and on
;; a key-combination

;; 1.0-beta2    07-Feb-1998
;; Finally smashed that drawing-past-end-of-buffer-sometimes-corrupt
;; bug (hurray!).
;; Added trimming of line-endings.

;; 1.0-beta1    05-Feb-1998
;; First beta
...
(defvar artist-draw-region-min-y 0
  "Line-number (buffer-relative) for topmost visited line for
draw-operation.")
(make-variable-buffer-local 'artist-draw-region-min-y)

(defvar artist-draw-region-max-y 0
  "Line-number (buffer-relative) for bottommost visited line for
draw-operation.")
...
(defvar artist-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "\C-c\C-e" 'artist-select-erase-char)
    (define-key map "\C-c\C-f" 'artist-select-fill-char)
    (define-key map "\C-c\C-l" 'artist-select-line-char)
    (define-key map "\C-c\C-o" 'artist-select-operation)
    (define-key map "\C-c\C-r" 'artist-toggle-rubber-banding)
    (define-key map "\C-c\C-t" 'artist-toggle-trim-line-endings)
    (define-key map "\C-c\C-c" 'artist-mode-off)
    (define-key map "\C-cl"    'artist-select-op-line)
    (define-key map "\C-cL"    'artist-select-op-straight-line)
    (define-key map "\C-cr"    'artist-select-op-rectangle)
    (define-key map "\C-cR"    'artist-select-op-square)
    (define-key map "\C-cs"    'artist-select-op-square)
    (define-key map "\C-cp"    'artist-select-op-poly-line)
    (define-key map "\C-cP"    'artist-select-op-straight-poly-line)
    (define-key map "\C-ce"    'artist-select-op-ellipse)
    (define-key map "\C-cc"    'artist-select-op-circle)
    (define-key map "\C-c\C-d" 'artist-select-op-erase-char)
    (define-key map "\C-cE"    'artist-select-op-erase-rectangle)
    (define-key map "\C-c\C-k" 'artist-select-op-cut-rectangle)
    (define-key map "\C-c\M-w" 'artist-select-op-copy-rectangle)
    (define-key map "\C-c\C-y" 'artist-select-op-paste)
    (define-key map "\C-cf"    'artist-select-op-flood-fill)
    (define-key map "\C-c\C-b" 'artist-submit-bug-report)
    map)
	...
	'<exit-fn>	; Function to call after filling is done.
         <draw-how>	; 'artist-do-continously:   do operation continously
			...
			;			    is 2: do rubberbanding.
                        ; If <draw-how> is 1 or 'artist-do-continously,
			; then only the draw-fn below needs to be specified.
...
;;
;; The car-cdr-things below are for extracting the <symbol>
;; in the (quote <symbol>).
;;
(defun artist-mt-get-tag (element)
  (car (cdr (aref element 0))))
...
(defsubst artist-fc-get-symbol (info-part)
  (car (cdr (aref info-part 1))))

(defsubst artist-fc-get-fn (info-part)
  (car (cdr (aref info-part 2))))
...
Drawing with keys
 ...
 \\[artist-key-set-point]		Does one of the following:
		For lines/rectangles/squares: sets the first/second end-point
		For poly-lines: sets a point (use C-u \\[artist-key-set-point] to set last point)
		When erase characters: toggles erasing
		When cutting/copying: Sets first/last end-point of rect/square
		When pasting: Pastes
...
(defvar	artist-coords-are-window-relative nil)
...
(defun artist-mode-init ()
  ...
  (make-local-variable 'artist-coords-are-window-relative)
  ...
  (setq artist-coords-are-window-relative t)
...
(defun artist-mode-exit ()
  "Exit artist-mode.
Does nothing but runs `artist-mode-exit-hook'."
  ;; Do nothing but run hooks
...
to retrieve information from that symbol's info-variant-part."
  (catch 'found
    (mapcar
     (lambda (element)
       (let ((element-tag (artist-mt-get-tag element)))
	 (if (eq element-tag 'graphics-operation)
	     (let* ((info-part     (artist-mt-get-info-part element))
		    (unshifted     (artist-go-get-unshifted info-part))
		    (shifted       (artist-go-get-shifted info-part))
		    (unshifted-sym (artist-go-get-symbol unshifted))
...
to retrieve information from that symbol's info-variant-part."
  (catch 'found
    (mapcar
     (lambda (element)
       (let ((element-tag (artist-mt-get-tag element)))
	 (if (eq element-tag 'function-call)
	     (let* ((info-part (artist-mt-get-info-part element))
...
;;
;; artist-current-line-window		get line number (counts from 0)
;; artist-current-line-buffer		get line number (counts from 0)
;;
;; artist-move-to-xy			calls one of the two following
;; artist-move-to-xy-in-window		move to (x,y) (0,0) is top left corner
;; artist-move-to-xy-in-buffer		move to (x,y) (0,0) is beg-of-buffer
;;
;; artist-get-char-at-xy		get char in at (x,y) (window or buf)
;;
...
(defsubst artist-current-line-window ()
  "Returns point's current line (window-relative)."
  (+ (count-lines (window-start) (point))
     (if (= (current-column) 0) 1 0)
     -1))

(defsubst artist-current-line-buffer ()
  "Returns point's current line (buffer-relative)."
  (+ (count-lines 1 (point))
     (if (= (current-column) 0) 1 0)
     -1))

(defsubst artist-move-to-xy-in-window (x y)
  "Moves to column X, at row Y from the top of the window."
  ;;
  ...
  ;;    second forward-line will return 
  ;;
  ...
  ;;    
  (forward-line (- y (artist-current-line-window)))
  (insert-char ?\n (forward-line (- y (artist-current-line-window))))
  (move-to-column (max x 0) 'force))

(defsubst artist-move-to-xy-in-buffer (x y)
  "Moves to column X, at row Y from the beginning of the buffer."
  (forward-line (- y (artist-current-line-buffer)))
  (insert-char ?\n (forward-line (- y (artist-current-line-buffer))))
  (move-to-column (max x 0) 'force))

(defsubst artist-move-to-xy (x y)
  "Moves to column X, at row Y.
Depending on the value fo `artist-coords-are-window-relative', interpret
Y as rows from top of the window or beginning of the buffer.

Also updates the variables `artist-draw-region-min-y' and
`artist-draw-region-max-y'."
  (if artist-coords-are-window-relative
      (artist-move-to-xy-in-window x y)
    (artist-move-to-xy-in-buffer x y))
  (let ((curr-y (artist-current-line-buffer)))
...
(defsubst artist-replace-string (string)
  "Replaces contents at point with STRING."
  (let ((char-list (append string nil))	; convert the string to a list
	(overwrite-mode 'overwrite-mode-textual)
	(fill-column 32765)		; Large :-)
	(blink-matching-paren nil))
    (while char-list
      (setq last-command-event (aref artist-replacement-table (car char-list)))
      (self-insert-command 1)
...
(defun artist-no-rb-unset-point1 ()
  ...
  (let ((x-now (artist-current-column))
	(y-now (artist-current-line-buffer))
...
;; Calculates char at intersection between two lines.
;;
...
;;		 X	 /		   X
;;		 X	 \		   X
;;		other combinations	use new char
;;
;; artist-intersection-char, takes new char, old char, returns char
;;
...
Output is a rectangle (which is currently a list consisting of
  ...
  (let* ((line1 (artist-draw-sline x1 y1 x2 y1))
...
(defun artist-ff-get-rightmost-from-xy (x y)
  ...
  (save-excursion
    (let ((char-at-xy (artist-ff-get-char-at-xy x y))
	  (last-x x))
      (setq x (1+ x))
      (while (and (not (artist-ff-too-far-right x))
		  (= char-at-xy (artist-ff-get-char-at-xy x y)))
	(setq last-x x)
	(setq x (1+ x)))
      last-x)))
...
(defun artist-ff-is-bottommost-line (x y)
  ...
  (save-excursion
    (goto-char (point-max))
    (beginning-of-line)
    (let ((last-line (if artist-coords-are-window-relative
			 (artist-current-line-window)
		       (artist-current-line-buffer))))
      (if (= (point) (point-max))
...
(defun artist-ff-get-char-at-xy (x y)
...
(defmacro artist-ff-push (x stack)
  (list 'setq stack (list 'cons x stack)))

(defmacro artist-ff-pop (stack)
  (list 'prog1
	(list 'car stack)
...
(defun artist-flood-fill (x1 y1)
  ...
  (if (not artist-fill-char-set)
      (error "No fill character is set"))

  (let ((stack nil)
	(input-queue nil)
	;; We are flood-filling the area that has this character.
	(c     (artist-ff-get-char-at-xy x1 y1)))
	...
	(artist-ff-push (artist-new-coord x1 y1) stack))

    (while (not (null stack))
      (let* ((coord (artist-ff-pop stack))
	...
	(while (and (>= x 0) (= c (artist-ff-get-char-at-xy x y)))
	  (if lines-above
	      (let ((c-above (artist-ff-get-char-at-xy x (- y 1))))
		(if (and (= c-above c) (/= c-above last-c-above))
		    (artist-ff-push (artist-new-coord x (- y 1)) stack))
	...
	(while (>= x x-leftmost)
	  (if lines-below
	      (let ((c-below (artist-ff-get-char-at-xy x (1+ y))))
		(if (and (= c-below c) (/= c-below last-c-below))
		    (artist-ff-push (artist-new-coord x (1+ y)) stack))
...
(defun artist-clear-arrow-points ()
  "Clears current end-points."
  (setq artist-arrow-point-1 nil)
  (setq artist-arrow-point-2 nil))

(defun artist-set-arrow-points-for-continously (x y)
  "Sets arrow-points for continous shapes at X,Y."
  (artist-clear-arrow-points))

(defun artist-set-arrow-points-for-poly (point-list)
  "Sets arrow-points for poly-shapes from POINT-LIST."
...
(defun artist-set-arrow-points-for-1point (x y)
  "Sets arrow-points for 1-point shapes at X,Y."
  (artist-clear-arrow-points))

(defun artist-set-arrow-points-for-2points (shape)
  "Sets arrow-points for 2-point shapes from SHAPE."
...
(defun artist-key-undraw-common () 
 ...
 (let ((draw-how (artist-go-get-draw-how-from-symbol artist-curr-go))
	...
	(row      (artist-current-line-buffer))
	(artist-coords-are-window-relative nil))
...
draw-region."
  ...
  (if (< max-y min-y)
	...
	(setq max-y tmp)))
  (save-excursion
    (let ((artist-coords-are-window-relative nil) ; coords are buffer-relative
	  (curr-y min-y))
      (while (<= curr-y max-y)
...
(defun artist-key-set-point-continously (x y)
  ...
  (let ((arrow-pred   (artist-go-get-arrow-pred-from-symbol artist-curr-go))
	...
	(if (artist-funcall arrow-pred)
	    (artist-set-arrow-points-for-continously x y)
	  (artist-clear-arrow-points))
  ...
  (let ((draw-fn      (artist-go-get-draw-fn-from-symbol artist-curr-go))
	...
	(if (not this-is-last-point)
	  ...
	  (progn
	    ...
	    (if (artist-funcall arrow-pred)
		(artist-set-arrow-points-for-poly artist-key-poly-point-list)
	      (artist-clear-arrow-points))
...
(defun artist-key-set-point-1point (x y)
  ...
  (let ((draw-fn      (artist-go-get-draw-fn-from-symbol artist-curr-go))
    ...
    (artist-funcall prep-fill-fn x y)
    (if (artist-funcall arrow-pred)
	(artist-set-arrow-points-for-1point x y)
      (artist-clear-arrow-points))
    (artist-funcall exit-fn x y))
  (setq artist-key-shape nil)
  (setq artist-key-is-drawing nil))
...
(defun artist-key-set-point-2points (x y)
  ...
  (let ((draw-fn      (artist-go-get-draw-fn-from-symbol artist-curr-go))
	...
	;;
	(if (artist-funcall arrow-pred)
	    (artist-set-arrow-points-for-2points artist-key-shape)
	  (artist-clear-arrow-points))
...
(defun artist-down-mouse-3 (ev)
  ...
  (interactive "e")
  (let ((artist-coords-are-window-relative t)
	(artist-curr-go 'erase-char))
...
Operation is done continously while the mouse button is hold down."
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
	 ...
	 (ev-start-pos (posn-col-row ev-start))
	 (x1           (car ev-start-pos))
	 (y1           (cdr ev-start-pos))
	 (shape))
	...
	(setq ev-start-pos (posn-col-row (event-start ev)))
	...
	(if (artist-shift-has-changed shift-state ev)
	    ...
	    (if (not (eq draw-how (artist-go-get-draw-how-from-symbol
    ...
    (if (artist-funcall arrow-pred)
	(artist-set-arrow-points-for-continously x1 y1)
      (artist-clear-arrow-points))
...
(defun artist-mouse-draw-poly (ev)
  ...
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
	...
	(cond
	 ...
	 ;; *** Mouse moved, button is down
	 ((and (mouse-movement-p ev) is-down)
	  ;; Draw shape from last place to this place
	  ;;
...
Operation is done once."
  ...
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
    ...
    (artist-funcall prep-fill-fn x1 y1)
    (if (artist-funcall arrow-pred)
	(artist-set-arrow-points-for-1point x1 y1)
      (artist-clear-arrow-points))
    (artist-funcall exit-fn x1 y1)))
...
(defun artist-mouse-draw-2points (ev)
  ...
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
    ...
    ;; Maybe set arrow-points
    (if (artist-funcall arrow-pred)
	(artist-set-arrow-points-for-2points shape)
      (artist-clear-arrow-points))
...
;;           is pressed.
;;           Examples: paste, a flood-fill mode
;;
;;
;; 2. To make it easier and more flexible to program new drawing
;;    modes, you might choose to specify a function to be called at the
;;    very beginning of the drawing phase (init-fn), a function to be
;;    called before filling and before the arrow-points are set
;;    (prep-fill-fn) and a function to be called at the very end of
;;    the drawing phase (exit-fn). For each of the cases below, the
;;    arguments given to the init-fn, prep-fill-fn and exit-fn are
;;    stated.
;;
...
;;
;;         If your mode can take arrows, then your
;;         artist-mouse-draw-xxx must set the variables
;;         `artist-arrow-point-1' and `artist-arrow-point-2'. If your
;;         mode does not take arrows, you must set the variables to
;;         nil. Use the accessors artist-arrow-point-* to create and
;;         inspect arrow-points.
;;
;;         You must call the init-fn, the prep-fill-fn and the exit-fn
;;         at the apropriate points.
;;
...
;;           `artist-key-do-continously-common' whenever the user moves
;;           around.
;;
;;         As for the artist-mouse-draw-xxx, these two functions must
;;         take care to do rubber-banding and to set arrows.
;;
