...
;;;
;;; Last modified: 31st August 1994
;;; Added sysdul extensions (Olav Bjørnås, omb@sysdeco.no)
;;;
;;; David Hughes <ukchugd@ukpmr.cs.philips.nl>
;;; Last modified: David Hughes 7th August 1995
;;; Version: 2.32
...
(fume-defvar-local
 fume-modeline-buffer-identification nil
 "Storage for the original mode-line-buffer-identification")
...
;;; Sysdul
(defconst fume-function-name-regexp-sysdul
   "^\\s-*\\(\\(\\(event\\|control\\)-\\)?\\(main\\)?procedure\\|\\(kontroll-\\)?\\(hoved\\)?prosedyre\\)\\s-*"
  "Expression to get Sysdul function names")
...
(defconst fume-function-name-regexp-alist
  '(;; Lisp
    ...
    ;; SYSDUL
    ;; Olav Bjørnås <omb@sysdeco.no>
    (sysdul-mode . fume-function-name-regexp-sysdul)
...
;;; Specialised routine to find the next SYSDUL PROCEDURE
;;;
(defun fume-find-next-sysdul-function-name (buffer)
  "Searches for the next sysdul procedure in BUFFER."
  (set-buffer buffer)
  (if (re-search-forward fume-function-name-regexp nil t)
      (let ((pos (point))
            ;; name may have "_" but must start with a letter
        ...
        (if (re-search-forward name-regexp eol t)
            ...
            (cons (buffer-substring pos (point)) pos)
          ;; rubbish found; skip to next function
          (fume-find-next-sysdul-function-name buffer)))))
...
(defconst fume-find-function-name-method-alist
  '((ada-mode        . fume-find-next-ada-function-name)
    ...
    (sysdul-mode     . fume-find-next-sysdul-function-name)
...
(defsubst fume-maybe-install-modeline-feature ()
  (cond ((fume-set-defaults)
         (or fume-modeline-funclist
             (memq 'fume-tickle-modeline post-command-hook)
             (fume-rescan-buffer))
         (make-variable-buffer-local 'post-command-hook)
         (add-hook 'post-command-hook 'fume-tickle-modeline)
...
(defun fume-tickle-modeline ()
  (let (fname (fume-display-in-modeline-p fume-display-in-modeline-p))
    (setq fname (and fume-display-in-modeline-p (fume-function-before-point))
          fume-display-in-modeline-p (not (null fname))
          modeline-buffer-identification
          (if fume-display-in-modeline-p
              (list
               fume-modeline-buffer-identification-1
               (concat " `" (fume-trim-string (format "%s" fname)) "'"))
  ...
  (setq fume-modeline-buffer-identification-1
        (let (newSeq)
          (mapcar
           (function
            (lambda (oldSeq)
              (or (and (stringp oldSeq)
                       (string-match "%[0-9]*f" oldSeq)
                       (aset
                        (setq newSeq (copy-sequence oldSeq))
                        (1- (match-end 0))
                        (string-to-char "b"))
                       newSeq)
                  oldSeq)))
...
(defsubst fume-install-rescan-buffer-trigger ()
  (cond ((null (memq 'fume-rescan-buffer-trigger post-command-hook))
         (make-variable-buffer-local 'post-command-hook)
         (add-hook 'post-command-hook 'fume-rescan-buffer-trigger 'append)
...
(if (and fume-running-xemacs (> emacs-minor-version 11))
    (defalias 'fume-event-window 'event-window)
  (defun fume-event-window (event)
    "More robust version of 'event-window'"
    ...
    (cond (fume-running-xemacs
           ...
           (let* ((count 0)
             ...
             (setq function-menu (cons
                                  (vector
                                   (concat "Rescan buffer :  "
                                           (buffer-name))
                                   (list 'fume-rescan-buffer
                                         (null use-menubar))
                                   t)
                                  (cons (vector "Display list of functions"
...
(defun fume-list-functions-help ()
  (interactive)
  (sit-for 0)
  (message "Goto function: o=%s, g=%s, G=%s,   [q=quit, h=help]"
           "other window"
           "this window"
           "one window"))
...
(defvar fume-list-keymap ()
  "Keys used in fume-listings")

(if fume-list-keymap ()
  (setq fume-list-keymap (copy-keymap global-map))
  (substitute-key-definition 'self-insert-command 'undefined fume-list-keymap)
  (define-key fume-list-keymap "q" 'fume-list-functions-quit)
  (define-key fume-list-keymap "h" 'fume-list-functions-help)
  (define-key fume-list-keymap "?" 'fume-list-functions-help)
  (define-key fume-list-keymap "g" 'fume-prompt-function-goto)
  (define-key fume-list-keymap "G" 'fume-prompt-function-goto-one-window)
  (define-key fume-list-keymap "o" 'fume-prompt-function-goto-other-window))
...
(defun fume-list-functions (&optional this-window)
  ...
  (cond ((or fume-function-name-regexp (fume-maybe-install-modeline-feature))
	     ...
	     (use-local-map fume-list-keymap)
