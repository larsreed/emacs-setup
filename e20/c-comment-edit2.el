...
;; $Id: c-comment-edit2.el,v 1.3 1995/10/14 13:57:35 jaalto Released jaalto $
...
;; LCD Archive Entry:
;; c-comment-edit|Kyle E. Jones|kyle_jones@wonderworks.com|
;; C Comment Edit in separate window.|
;; 14-Oct-1995|1.3|~/misc/c-comment-edit2.el.Z|
;;
...
;;{{{ code: Byte compiler hack

;;; ................................................... &byte-compiler ...
;;; 19.28 compiler and 19.30 [RMS told so] are buggy, silence them.

(eval-when-compile
  (defvar font-lock-mode nil "dummy")
  (defun font-lock-fontify-region (&rest x) (error "not exist"))
  )
...
"
  ...
  (let* ((cs	   (or comment-start ""))
	 (ce	   (or comment-end ""))
	 (comment  (concat cs " "  ce))
	 )
    (if (comment-area cs ce)
	(c-comment-edit nil)

      ;;  The catch is, that we first create a commend and then
...
Indentation will be the same as the original."
  ...
  (let ((tuple		(find-c-comment-buffer))
	...
	(save-excursion
	  ...
	  (if (and (boundp 'font-lock-mode)   ;; is it loaded?
		   font-lock-mode)
	      (font-lock-fontify-region c-comment-edit-beg-c
					c-comment-edit-end-c))
	  )
	)
