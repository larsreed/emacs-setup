...
(when (memq 'reg-generic-mode generic-extras-enable-list)
  ...
  (define-generic-mode 'reg-generic-mode
    ...
    '("\\.[rR][eE][gG]\\'")
    (list 'generic-reg-mode-setup-function)
    "Generic mode for MS-Windows Registry files.")
  
  (defvar reg-generic-mode-hook nil)
  (defun generic-reg-mode-setup-function ()
    (setq imenu-generic-expression
	  '((nil "^\\s-*\\(.*\\)\\s-*=" 1)))
    (run-hooks 'reg-generic-mode-hook))
  )
...
(define-generic-mode 'bat-generic-mode
  ...
  (defvar bat-generic-mode-hook nil)
  (defun generic-bat-mode-setup-function ()
    ...
    (run-hooks 'bat-generic-mode-hook)
    )
  )
