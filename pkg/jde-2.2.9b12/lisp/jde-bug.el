...
;; $Revision: 1.84 $ $Date: 2002/09/11 03:37:43 $ 
...
(defvar jde-bug-menu-spec
  (list "JDEbug"
	...
	(list
	 ...
	 ["Local Variables"           
	  jde-bug-display-local-variables 
	  (and 
	   (jde-dbs-debugger-running-p)
	   (jde-dbs-get-target-process))] 
...
(defun jde-bug-display-variable () 
  ...
  (let ((qualified-expr 
	 (oref (semantic-analyze-current-context (point)) prefix)))
    (if qualified-expr
	(jde-bug-evaluate-expression 
	 (mapconcat 'identity qualified-expr ".")))))
