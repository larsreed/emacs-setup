...
;; $Revision: 1.16 $ $Date: 2002/06/17 07:24:08 $ 
...
current project."
  (let (jdb)
    ...
    (oset 
     ...
     :path
     (expand-file-name 
      (concat "bin/" (oref jdb :exec-name))
      (jde-get-jdk-dir)))
    (if (eq system-type 'windows-nt)
	(oset jdb :path (concat (oref jdb path) ".exe")))
    (if (file-exists-p (oref jdb path))
	jdb
      (error "Cannot find %s" (oref jdb :path)))))
