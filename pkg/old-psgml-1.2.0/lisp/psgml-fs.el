    ...
    (when entity-map
      (setq text
	    (loop for (name val) on entity-map by 'cddr
		  thereis (if (equal name (psgml-entity-name entity))
			      val))))
    (unless text
      (setq text (psgml-entity-text entity)))
...
(defun fs-engine (e)
  (fs-do-style e
	       (cdr (or (assoc (psgml-element-gi e) fs-style)
			(assq t fs-style)))))
...
(defun fs-element (&rest moves)
  ...
  (let ((element fs-current-element))
    (while moves
      (case (pop moves)
	(parent (setq element (psgml-element-parent element)))
	(next   (setq element (psgml-element-next element)))
	(child  (setq element (psgml-element-content element)))))
    element))


(defun fs-attval (name &optional element)
  (psgml-element-attval (if element element (fs-element))
		       name))

(defun fs-child-number (&optional element)
  (let* ((element (or element (fs-element)))
	 (parent (psgml-element-parent element))
	 (child  (psgml-element-content parent))
	 (number 0))
    (while (and child (not (eq child element)))
      (incf number)
      (setq child (psgml-element-next child)))
    number))


(defun fs-element-with-id (id)
  (block func
    (let ((element (psgml-top-element)))
      (while (not (psgml-off-top-p element))
	(let ((attlist (psgml-element-attlist element)))
	  (loop for attdecl in attlist
		if (eq 'ID (psgml-attdecl-declared-value attdecl))
		do (if (equalp id (psgml-element-attval element
						       (psgml-attdecl-name attdecl)))
		       (return-from func element))))
	;; Next element
	(if (psgml-element-content element)
	    (setq element (psgml-element-content element))
	  (while (null (psgml-element-next element))
	    (setq element (psgml-element-parent element))
	    (if (psgml-off-top-p element)
		(return-from func nil)))
	  (setq element (psgml-element-next element)))))
    nil))
