...
(eval-and-compile
  (autoload 'psgml-do-set-option "psgml-edit"))
  ...
  (setq entries
	(loop for ent in entries collect
	      (vector (car ent)
		      ...
		      t)))
  (cond ((> (length entries) psgml-max-menu-size)
	 (setq entries
	       (loop for i from 1 while entries collect
		     (let ((submenu
			    (subseq entries 0 (min (length entries)
						   psgml-max-menu-size))))
		       (setq entries (nthcdr psgml-max-menu-size
					     entries))
		       (cons
			(format "%s '%s'-'%s'"
				title
				(psgml-range-indicator (aref (car submenu) 0))
				(psgml-range-indicator
				 (aref (car (last submenu)) 0)))
			submenu))))))
  (psgml-lucid-get-popup-value (cons title entries)))
...
(defun psgml-lucid-get-popup-value (menudesc)
  (let ((value nil)
	(event nil))
    (popup-menu menudesc)
    (while (popup-up-p)
...
(defun psgml-popup-multi-menu (pos title menudesc)
  "Display a popup menu.
...
(defun psgml-make-options-menu (vars)
  (loop for var in vars
	for type = (psgml-variable-type var)
...
(unless (or (not (boundp 'emacs-major-version))
  ...
  (loop for ent on psgml-main-menu
	if (vectorp (car ent))
	do (cond
	    ((equal (aref (car ent) 0) "File Options >")
	     (setcar ent
		     (cons "File Options"
			   (psgml-make-options-menu psgml-file-options))))
	    ((equal (aref (car ent) 0) "User Options >")
	     (setcar ent
		     (cons "User Options"
			   (psgml-make-options-menu psgml-user-options)))))))


;;;; Key definitions

(define-key psgml-mode-map [button3] 'psgml-tags-menu)
...
(defun psgml-set-face-for (start end type)
  (let ((face (cdr (assq type psgml-markup-faces)))
    ...
    (loop for e being the extents from start to end
	  do (when (extent-property e 'psgml-type)
	       (cond ((and (null o)
			   (eq type (extent-property e 'psgml-type)))
		      (setq o e))
		     (t (delete-extent e)))))
    ...
    (cond (o
	  ...
	  (face
	   ...
	   (set-extent-property o 'psgml-type type)
...
(defun psgml-set-face-after-change (start end &optional pre-len)
  ...
  (when psgml-set-face
    (let ((o (extent-at start nil 'psgml-type)))
      (cond
       ((null o))
