...
(defun psgml-map-element-modify (el-fun element)
...
leaves the element with no start-tag some elements may be ignored."
  (let ((level				; level in the element tree
	...
	(setq element (psgml-find-element-of (psgml-element-start element)))
	(setq tick (buffer-modified-tick)))
      (cond
       ;; Map content if any
       ((setq next (psgml-element-content element))
	(incf level))
       ;; If in a sub-tree, move to next element
       (t
	(while (and (> level 0)
		    (null (setq next (psgml-element-next element))))
...
(defun psgml-map-content (element element-fun
...
Also calling DATA-FUN, if non-nil, with data in content."
  ...
  (psgml-need-dtd)
  (psgml-element-end element)		; Make sure all content is parsed
  (let ((main-buffer-max (point-max)))
    (save-excursion
      (psgml-set-parse-state element 'start)
      (when (eobp) (psgml-pop-entity))
      (when (eolp) (forward-char 1))
      (psgml-parse-data main-buffer-max data-fun pi-fun entity-fun)
      (let ((c (psgml-tree-content element)))
	(while c
	  (psgml-pop-all-entities)
	  (funcall element-fun c)
	  (psgml-set-parse-state c 'after)
	  (psgml-parse-data main-buffer-max data-fun pi-fun entity-fun)
	  (setq c (psgml-tree-next c)))))
    )
  (psgml-pop-all-entities))

(defun psgml-parse-data (psgml-goal psgml-data-function psgml-pi-function
				  psgml-entity-function)
  (let ((psgml-throw-on-element-change 'el-done))
    (catch psgml-throw-on-element-change
      (psgml-with-parser-syntax
       (psgml-parser-loop nil)))))

;;;; Entity management

(defun psgml-push-to-string (string)
...
of the new entity with point at the first character.
    Use `psgml-pop-entity' to exit from this buffer."
  (psgml-push-to-entity (psgml-make-entity "#STRING" 'text string)))
