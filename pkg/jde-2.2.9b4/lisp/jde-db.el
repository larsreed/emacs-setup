...
;; $Revision: 1.79 $ $Date: 2001/09/30 05:29:32 $ 

;; Author: Paul Kinnucan <paulk@mathworks.com>
;; Maintainer: Paul Kinnucan
;; Keywords: java, tools

;; Copyright (C) 1997, 2000, 2001 Paul Kinnucan.
...
(defun jde-widget-validate-path (widget)
  (if (not (file-exists-p (widget-get widget :value)))
      (progn
	(widget-put widget :error "Custom debugger path does not exist.")
	widget)))
	   
(defun jde-widget-match-path (widget path) 
  (file-exists-p path))

;;(makunbound 'jde-debugger)
(defcustom jde-debugger (list "jdb (new)")
  "Specify the debugger you want to use to debug Java programs.  Select
JDEbug, if you want to use the JDE's builtin debugger.  Select jdb (new), if
you want to use the version of jdb that comes with JDK 1.3 (or later)
or the Linux version of JDK 1.2.2.  Select jdb (old), if you want to use
...
(makunbound 'jde-db-source-directories)
...
one path in the same edit field. You'll only confuse JDE.
...
(defcustom jde-db-option-attach nil
  "Nonnil if you want the debugger to attach itself to a running process."
  :group 'jde-db-options
  :type 'boolean)
...
(defmethod jde-db-attach-id-arg ((this jde-db-debugger))
  (list "-password"
...
(defclass jde-db-jdb (jde-db-debugger)
  ((filename        :initarg :filename
		    ...
		    :initform "jdb"
	            :documentation 
	            "File name of the jdb executable.")
   (marker-regexp   :initarg :marker-regexp
		    :type string
		    :documentation 
		    "Filter for parsing debugger output.")
...
(defmethod jde-db-jdb-launch ((this jde-db-jdb) app-class marker-filter find-file)
  (if (or
      ...
      (let* ((source-directory default-directory)
	    ...
	    (prog-args
	     (if jde-db-option-attach			  
		 (append 
		  (jde-db-attach-id-arg this)
		  (jde-db-host-arg this))
	       (append 
	...
	(oset this :buffer-name (concat "*debug" app-class "*"))
	...
	(save-excursion
	  ...
	  (comint-exec (oref this :buffer)
		       (if jde-db-option-attach "attach"
			 app-class)
		       (oref this :filename)
		       nil
		       prog-args)
...
(defmethod jde-db-jdb-goto-stack-frame ((this jde-db-jdb))

    (if (string-match "^[-a-zA-Z0-9_$]+\\[\\([0-9]+\\)\\] " (oref this :marker-acc))
	(oset this :stack-depth (match-string 1 (oref this :marker-acc))))
    ...
    (if (string-match 
	 (concat "^  \\[" 
		 (oref this :stack-depth)
		 "\\] .*(\\([^\$\n]*\\).*:\\([0-9]+\\))")
	 (oref this :marker-acc))
	(let ((marker (match-string 0 (oref this :marker-acc)))
	      (class (match-string 1 (oref this :marker-acc)))
	      (line-no (string-to-int (match-string 2 (oref this :marker-acc)))))
...
(defclass jde-db-oldjdb-class (jde-db-jdb)
  ()
  (:allow-nil-initform t)
"Class of generic jdb debuggers")

(defmethod initialize-instance ((this jde-db-oldjdb-class) &rest fields)
  "Constructor for old jdb debugger."
  ...
  (oset this :class-index 1)
  (oset this :line-index 2))

(defvar jde-db-the-debugger (jde-db-jdb "jdb"))
...
(defun jde-db-fixup-input ()
  ...
  ;;(message "checking string %s" (oref jde-db-the-debugger :marker-acc))
  (if (string-match "^.*: \\([-a-zA-Z0-9_$]+\\[[0-9]+\\] \\)thread=" 
                    (oref jde-db-the-debugger :marker-acc))
      (oset jde-db-the-debugger :marker-acc
            (concat (match-string 1 (oref jde-db-the-debugger :marker-acc))
                    (substring (oref jde-db-the-debugger :marker-acc) 0 (match-beginning 1))
                    (substring (oref jde-db-the-debugger :marker-acc) (match-end 1)))))
...
(defun jde-db-marker-filter (input)
  ...
  (let* ((output "")
	 (marker-regexp (oref jde-db-the-debugger :marker-regexp))
	 (marker-regexp-class-index (oref jde-db-the-debugger :class-index))
	 (marker-regexp-line-index (oref jde-db-the-debugger :line-index)))

    ;; (message (concat "jdb output:" input))
    ;; (message (concat "acc = " jde-db-marker-acc))
    
    ;; Process all the complete markers in this chunk.
    (if (string-match marker-regexp (oref jde-db-the-debugger :marker-acc))
	;; Extract the frame position from the marker.
	(let ((premarker (substring 
			  (oref jde-db-the-debugger :marker-acc) 0 (match-beginning 0)))
	      (marker (substring (oref jde-db-the-debugger :marker-acc) 
	      ...
	      (rest (substring (oref jde-db-the-debugger :marker-acc) (match-end 0))))
	  ...
	  (oset jde-db-the-debugger :marker-acc rest))))

    ;; Handle case where there is no line number info in current class.
    (if (string-match jde-db-nodebug-marker-regexp (oref jde-db-the-debugger marker-acc))
	(let ((premarker (substring 
			  (oref jde-db-the-debugger :marker-acc) 0 (match-beginning 0)))
	      (marker (substring (oref jde-db-the-debugger :marker-acc)
				 (match-beginning 0) (match-end 0)))
	      (pc (substring (oref jde-db-the-debugger :marker-acc)
			     (match-beginning 1) (match-end 1)))
	      (rest (substring (oref jde-db-the-debugger :marker-acc) (match-end 0))))

	  (setq output (concat premarker marker))))

    (jde-db-jdb-goto-stack-frame jde-db-the-debugger)
	...
	(progn
	  ...
	  (setq output (concat output 
			       (substring (oref jde-db-the-debugger :marker-acc)
					  0 (match-beginning 0))))

	  ;; Everything after, we save, to combine with later input.
	  (oset jde-db-the-debugger 
		:marker-acc
		(substring (oref jde-db-the-debugger :marker-acc) (match-beginning 0))))

      (setq output 
	    (concat output (oref jde-db-the-debugger :marker-acc)))
      (oset jde-db-the-debugger :marker-acc ""))    
...
it returns the file's path. Otherwise, it returns nil."
  (assert 
   ...
   ;; Kevin Burton's (too?) strict version. Only alphabetic and underscore chars
   ;; allowed in package and class names.
   ;;(string-match "^\\(\\(\\([a-z_]\\)*[.]\\)*\\)\\(\\([a-zA-Z_]\\)+$\\)" class)
   nil
   "\"%s\" is an invalid qualified class name." class)
  (let* ((package-name
     ...
     (if source-dir
       ...
       (message "JDE error: Could not find source for \"%s\". See `jde-db-source-directories' for more information." class)
       nil)))
...
CLASS is the name of the class you are debugging."
  ...
  (if (string-match "XEmacs\\|Lucid" emacs-version)
    ...
    (jde-db-jdb-launch jde-db-the-debugger
     (if (not jde-db-option-attach)
  ...
  (jde-db-def gud-where  "where"          "\C-w" "Display location on the stack.")
...
(defun jde-db-applet-init (applet-class marker-filter find-file applet-doc-path)
...
file in the current buffer."
  ...
  (let ((applet-doc-path 
    ...
    (if applet-doc-path 
	(jde-db-applet-internal applet-doc-path) 
