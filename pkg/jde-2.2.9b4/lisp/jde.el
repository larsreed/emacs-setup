...
;; $Revision: 1.201 $ $Date: 2001/10/19 09:53:21 $ 
...
;;;###autoload
(defconst jde-version "2.2.9beta4"
  "JDE version number.")
...
(defcustom jde-jdk nil
  "Specifies the version of the JDK to be used
to develop the current project. The version must
be one of the versions listed in the 
`jde-jdk-registry'. If you specify the null string
(the default), the JDE uses the value
of the Java version environment variable (see
`jde-java-enviroment-variables', if set; otherwise,
the first JDK located on the PATH environment variable."
  :group 'jde-project
  :type 'symbol)
...
(defcustom jde-jdk-registry nil
  "Specifies the versions and locations of the JDKs installed on your system."
  :group 'jde-project
  :type '(repeat
	  (cons 
	   :tag "JDK"
...
(defcustom jde-jdk-doc-url "http://java.sun.com/j2se/1.3/docs/index.html"
	 ...
	 (menu (if (fboundp 'easy-menu-create-menu)
		   (easy-menu-create-menu 
		    (car jde-new-buffer-menu) (cdr jde-new-buffer-menu))
                 ;; This looks like an attempt to support some archaic
		 ;; version of XEmacs. Let's see if we can get away
...
to jar or zip files in those directories. This function
assumes that the existing paths are already normalized."
...
(defun jde-save-close-buffer (project)
  ...
  (let* ((buffer 
	  (find-if 
	   (lambda (buf)
	     (let* ((directory-sep-char ?/) ;; XEmacs defaults to a backslash
		    (buf-file-name (buffer-file-name buf)))
	       (and buf-file-name
		    (string= project (expand-file-name buf-file-name)))))
	     (buffer-list)))
