...
;; $Revision: 1.67 $ $Date: 2001/10/01 18:40:36 $ 
...
(defvar jde-bug-menu-spec
  (list "JDEbug"
	...
	["Step Into All"              jde-bug-step-into-all 
	                              (jde-dbs-target-process-steppable-p)]
	...
	"-"
	;; Added by lea
        ["Toggle Breakpoint"          jde-bug-toggle-breakpoint t]
...
(defun jde-bug-remove-breakpoint-highlight ()
  (if jde-xemacsp
	...
	(let ((overlay (car overlays)))
	  (if (eq (overlay-get overlay 'face) 'jde-bug-breakpoint-marker)
...
(defun jde-bug-toggle-local-variables () "Enables and disables the retrieval
of the local variables. It toggles the value of the variable
`jde-bug-local-variables'"
  (interactive)
  (setq jde-bug-local-variables (not jde-bug-local-variables))
  (if (and jde-dbo-current-process jde-dbo-current-thread-id
           jde-bug-local-variables)
...
(defun jde-bug-toggle-stack-info () "Enables and disables the retrieval
of stack info. It toggles the value of the variable
`jde-bug-stack-info'"
  (interactive)
  (setq jde-bug-stack-info (not jde-bug-stack-info))
  (if (and jde-dbo-current-process jde-dbo-current-thread-id jde-bug-stack-info)
