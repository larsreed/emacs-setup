...
;; Keywords: java, tools
;; VC: $Id: jde-java-font-lock.el,v 1.11 2001/09/18 15:03:52 jslopez Exp $
...
the search.  The match found must not extend after that position."
  (let ((here (point))
    ...
    (while (and (re-search-forward regexp end t)
                (not (setq ok (eq (get-text-property
      ...
      (setq md (match-data)
            ok (and (re-search-backward "^\\s-*\\(/[*][*]\\)" nil t)
                    (eq (get-text-property
                         (goto-char (match-beginning 1)) 'face)
                        'font-lock-comment-face)
                    (forward-comment 1)
                    (< p (point))
                    (setq here p)))
...
(defun jde-java-font-lock-html-bold-matcher (end)
  "Font lock matcher for HTML B and STRONG style in javadoc comments.
Limit search to END position."
  (jde-java-font-lock-search-in-javadoc
...
(defconst jde-java-font-lock-html-bold-keyword
  '(jde-java-font-lock-html-bold-matcher
    2 jde-java-font-lock-bold-face t)
  "Font lock keyword for javadoc HTML B and STRONG style.")

(defun jde-java-font-lock-html-italic-matcher (end)
  "Font lock matcher for HTML I and EM style in javadoc comments.
Limit search to END position."
  (jde-java-font-lock-search-in-javadoc
   "<\\([Ii]\\|[Ee][Mm]\\)>\\([^<]*\\)</\\([Ii]\\|[Ee][Mm]\\)>"
   end))

(defconst jde-java-font-lock-html-italic-keyword
  '(jde-java-font-lock-html-italic-matcher
    2 jde-java-font-lock-italic-face t)
  "Font lock keyword for javadoc HTML I and EM style.")

(defun jde-java-font-lock-html-underline-matcher (end)
  "Font lock matcher for HTML U style in javadoc comments.
Limit search to END position."
  (jde-java-font-lock-search-in-javadoc
   "<[Uu]>\\([^<]*\\)</[Uu]>"
   end))

(defconst jde-java-font-lock-html-underline-keyword
  '(jde-java-font-lock-html-underline-matcher
    1 jde-java-font-lock-underline-face t)
  "Font lock keyword for javadoc HTML U style.")

(defun jde-java-font-lock-html-code-matcher (end)
  "Font lock matcher for HTML CODE style in javadoc comments.
Limit search to END position."
  (jde-java-font-lock-search-in-javadoc
   "<[Cc][Oo][Dd][Ee]>\\([^<]*\\)</[Cc][Oo][Dd][Ee]>"
   end))

(defconst jde-java-font-lock-html-code-keyword
  '(jde-java-font-lock-html-code-matcher
    1 jde-java-font-lock-code-face t)
  "Font lock keyword for javadoc HTML CODE style.")

(defun jde-java-font-lock-html-pre-matcher (end)
  "Font lock matcher for HTML PRE style in javadoc comments.
Limit search to END position."
  (jde-java-font-lock-search-in-javadoc
   "<[Pp][Rr][Ee]>\\([^<]*\\)</[Pp][Rr][Ee]>"
   end))

(defconst jde-java-font-lock-html-pre-keyword
  '(jde-java-font-lock-html-pre-matcher
    1 jde-java-font-lock-pre-face t)
  "Font lock keyword for javadoc HTML PRE style.")
