...
list. It returns a string if the fqn was found, or null otherwise."
  (if (jde-complete-class-exists name)
      name
    (let ((importlist (jde-complete-import-list))
...
informations on the completion."
  ...
  (let ((class-info (jde-complete-get-from-cache name))
    ...
    (when (not class-info)
      ...
      (setq public-methods
            (jde-complete-invoke-getClassInfo
    ...
    (setq class-info (jde-complete-get-accessible-info class-info access-level name))
...
(defun jde-complete-get-accessible-info (class-info access name)
  "Takes a list of class info in this format \(list \(list public info\)
\(list protected info\) \(list package info\) \(list private info\)\).
Each info list is in the format
\(list \(list fields\) \(list constructors\) \(list methods\) \(list inner classes\)\).
This method will return a list concatenating the
fields, methods, and inner classes for the access level."
  (let ((public (nth jde-complete-public class-info))
        ...
        answer fields methods classes packagep)
    (if package-name 
    ...
    (setq fields (append (nth jde-complete-fields public)
                         ...
                         (if (>= access jde-complete-private)
    ...
    (setq answer (list fields nil methods classes))
    answer))
...
jde.util.Completion.getClassInfo function."
  (let (result tmp current prev end-paren end-parens)
    ...
    ;; get inner classes
    (setq tmp (nth jde-complete-classes classinfo))
    (if tmp 
        (while tmp
    ...
    (setq end-parens
          (if (null jde-complete-insert-method-signature)
              (if (null jde-complete-add-space-after-method)
                ...
                " ()")
            ""))
    (setq tmp (nth jde-complete-methods classinfo))
    (setq prev nil)
    (if (null jde-complete-unique-method-names)
        (while tmp
          (let* ((type (car (cdr (car tmp))));;method type i.e. boolean
                 (exceptions (jde-get-exceptions (last (car tmp))))
            ...
            (setq result
                  (append
                   (list (cons
                          (concat
                           method " : " type
                           (if exceptions (concat " throws " exceptions)))
                          (concat
                           (if (null jde-complete-insert-method-signature)
                               current
                             method)
...
(defun jde-get-exceptions (exceptions)
  "Takses as an argument a list of exceptions an return a string of them or nil"
  (if (and (listp exceptions) (car exceptions))
      (let ((exs ""))
        (while exceptions
          (setq exs (concat exs (caar exceptions)))
          (setq exceptions (cdr exceptions))
          (if exceptions (setq exceptions (concat exs ", "))))
        exs)
    nil))
...
(defun jde-complete-complete-cycle ()
  ...
  (let (elem tmp)
    ...
    (if tmp
	(progn
	  (delete-region jde-complete-current-beginning
  ...
  (let (qualified-name chop-pos temp answer)
    (setq answer
          (cond
           ...
           ((string= "super" expr)
            (jde-complete-get-qualified-name
           ...
           ((and (string= "this.this" expr)
                 (setq qualified-name (jde-complete-get-inner-class
           ...
           (t
            (let (to-complete
              ...
              (cond
               ((eq last-char ?\))
                (let* ((result
                        (jde-complete-isolate-before-matching-of-last-car
                         expr))
                  ...
                  (if qualified-name
                    ...
                    (if jde-complete-current-list
                        (progn
                          (setq to-complete (car
                                             (car jde-complete-current-list)))
                  ...
                  (if result
                      (let ((count 0) type)
                        (while (string-match ".*\\[\\]" result)
                          (setq result (substring result 0 (- (length result)
                                                              2)))
                          (setq count (1+ count)))
                        ...
                        (let (work)
                          (setq type
                                (cond
                                 ...
                                 ((setq work
                                        (jde-complete-get-inner-class-name
                        ...
                        (if type
                            (progn
                              (while (> count 0)
                                ...
                                (setq count (1- count)))
                              (jde-complete-transform-array-classes-names
                               type))
                          (if (y-or-n-p
...
otherwise."
  (let (class-name this-full-class-name result)
    (setq this-full-class-name (jde-complete-get-qualified-name
                                (jde-parse-get-class-at-point)))
    (cond
     (qualifier
     ...
     ((jde-complete-class-exists (setq class-name
...
This function displays the signature of a method completion as specified by
`jde-complete-display-current-signature' This command uses the Beanshell to
run Java code that in turn uses Java reflection to determine the methods and
fields defined by the class of the object at point. This command starts the
 Beanshell if necessary. Hence, you may experience a slight delay when using
 this command for the first time in a session or when completing a field or
 method of an object that has many methods and fields. See
 `jde-complete-at-point-menu' for a version of this command that lets you
 select the desired completion from a popup menu."
  (interactive)
  (if (and
    ...
    (progn 
      (let (completions
        ...
        (if access
            (setq completions (jde-complete-find-completion-for-pair pair nil
                                                                     access))
          (setq completions (jde-complete-find-completion-for-pair pair)))
        ;;if the jde-complete-current-list  is nil check if the method is
        ;;in the current class(this)
        (if (null completions)
            (setq completions 
                  (jde-complete-find-completion-for-pair (list (concat
                                                                "this."
                                                                (car pair)) "")
                                                         nil
    ...
    (setq pair
          (if (= (length index-alist) 1)
            ...
            (imenu--mouse-menu index-alist   
                               (jde-cursor-posn-as-event) ;Popup window at 
                               (or title "Completion")))) ;text cursor
    (setq name (cdr pair))
...
(defun jde-complete-at-point-menu ()
  "Completes the method or field name at point.
This command displays a popup menu listing the potential completions for the
 name at point. Selecting a completion causes the command to use the
completion to complete the name at point. See `jde-complete-at-point' for a
  ...
  (let* ((pair (jde-complete-java-variable-at-point))
    ...
    (if (string= (car pair) "this")
      ...
      (if (string= (car pair) "super")
          (setq access jde-complete-protected)))
    (if access 
        (setq completion-list (jde-complete-find-completion-for-pair pair nil
                                                                     access))
...
(defun jde-complete-popup-message (message buffer-or-name)
  "Split up the current window horizontally, the new buffer
is exactly 2 lines in height. Message is inserted in the new buffer.
Succesive calls to this method with the same buffer-or-name will
delete the text inside the buffer and replace it with
message. Message should not be longer than a line."
  (interactive)
...
(defun jde-complete-place-cursor ()
  "Places the cursor in between the parenthesis after a completion. This is
only done for methods that contain parameters, for all the other completions
the cursor is place at the end."
