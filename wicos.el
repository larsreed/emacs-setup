...
;; 
;; This package was called screens.el, but due to name conflict with 
;; Epoch and Emacs-19 screens the name has been changed.
;;
;; If you have a version of screens.el please remove it and use this instead!
;;
;; v1.4 ; 22 Sep 1992
;;
;; Copyright 1992 Heikki Suopanki
;;
...
;  '(lambda() 
;     (wico-set-type)
;     (let ((mode (aref wico-modes wico-this-wico))
;	   (alist wico-colors-alist) (colors nil))
...
;;;  the normal binding (M-o) which can now have different binding.
...
;; Sep 22 1992 v1.4
;;   * Epoch support
;;   * mode line shows only name
;;      -- suopanki@phoenix.oulu.fi
;; Aug 29 1992
;;   * a severe bug in wico-set-window-configuration fixed
;;   * user can define wico names
;;   * some internal functions renamed or removed
;;   * the way wico-types is created changed
;;      -- suopanki@phoenix.oulu.fi
;; Aug 27 1992
;;   * wico # not shown automatically anymore
;;   * wico menu now looks more like buffer menu 
;;      -- suopanki@phoenix.oulu.fi
;; Aug 2 1992  wicos.el v1.35
;;   *  screens.el is now called wicos.el, all function names changed too
;;   *  number of wicos is not limited anymore
;;   *  buffers associated with wicos
;;   *  named wicos
;;   *  other little changes
;;      -- suopanki@stekt.oulu.fi
;; Jul 10 1992
;;   *  screen-mode-line/global-mode-string
;;      -- tm@mentor.dk (Tonny Madsen)
;; May 20 1992
;;   *  screen-reset-window-config
;;   *  screen-x-menu
;;   *  screen-x-show-list
;;      -- hrue@sima.sintef.no, suopanki@stekt.oulu.fi
;; Apr 9 1992
;;   * resets point correctly for every window in each wico
;;     -- franl@centerline.com  (Fran Litterio)
;;   * screen-show-list modified
;;     -- suopanki@stekt.oulu.fi
;; Apr 7 1992  v1.3
;;   * rewritten version
;;     -- suopanki@stekt.oulu.fi
...
"*Shows the wico name in the mode line if t.")
...
"*How many buffers is associated with each wico")
...
(defun wico-update-mode-line ()
  (let ((name ""))
    (wico-set-type)
    (setq name (aref wico-types wico-this-wico))
...
Move to the line where desired wico is and type \\[wico-menu-select]
to select that wico.
...
(defun wico-x-menu (arg)
  (let ((selection
	 (x-popup-menu
	  arg
	  '("wico menu"
	    ("wico menu"
	     ("x : x wico menu"           . wico-x-show-list)
	     ("n : next"                    . wico-next)
