...
;; Sat May 31 00:00:00 BST 1997  Dave Love <d.love@dl.ac.uk>
;; 
;; 	   * (browse-url-gnudoit-program, browse-url-gnudoit-args,
;; 	   browse-url-generic-program, browse-url-generic-args): New
;; 	   variables.
;;	   * (browse-url-w3-gnudoit): New procedure.
;; 	   * (browse-url-mmm): New location of `remote' file for MMM
;; 	   0.4.
;;	   * (browse-url-generic): New procedure.
;; 
;; Sat Apr  5 23:54:09 BST 1997  Denis Howe <dbh@doc.ic.ac.uk>
;; 
;; 	   * (browse-url-netscape): Test for win32.
;; 
;; Tue Apr  1 00:27:02 BST 1997  Denis Howe <dbh@doc.ic.ac.uk>
;; 
;; 	   * (browse-url-url-at-point): assume mailto: if URL contains @.
;; 
;; Sat Mar 29 22:52:36 GMT 1997  Denis Howe <dbh@doc.ic.ac.uk>
;; 
...
;; 
;; 
;; Fri 23 Aug 12:00:00 1996 H}vard Fosseng <havard.fosseng@usit.uio.no>
;; 
...
;; Define some dummy functions for old Emacsen.
;; Avoid compile warnings.
(eval-when-compile
  ;; These prevent compile warnings but screw w3
  ;; (or (fboundp 'selected-device)   (defun selected-device nil nil))
  ;; (or (fboundp 'device-type)       (defun device-type (a) nil))
  ;; (or (fboundp 'device-connection) (defun device-connection (a) nil))
  ;; xemacs, early Emacs 19 compatibility:
  (or (fboundp 'buffer-substring-no-properties)
      (defun buffer-substring-no-properties (a b)
	(buffer-substring a b))))

;; Fake defcustom, defgroup for Emacs 19.29 (borrowed from w3):
...
Display only the currently visible part of BUFFER
(from a temporary file) if buffer is narrowed."
  (interactive)
  (save-excursion
...
(defun browse-url-choose-browser (url &rest args)
  "Pass URL to a browser function chosen according to the
association list in variable ``browse-url-browser-function''."
...
browse-url."
  ...
  (save-excursion
    ...
    (let ((url (browse-url-url-at-point)))
      (if (string-equal url "") (error "No URL found"))
      (browse-url url))))
...
(defun browse-url-emacs-display ()
  "Return the X display Emacs is running on if different from the
