  ...
  (if (> (length menulist) imenu-max-items)
	...
	(cons title
	      (mapcar
	       (function
		(lambda (menu)
		  (let* ((subtitle (car (car menu)))  ;; [LRE]
			 (subtitle (if (and (stringp subtitle)
					    (> (length subtitle) 0))
				       subtitle
				     title)))
		    (cons (format "(%s-%d)" subtitle (setq count (1+ count)))
			menu))))
...
Returns t for rescan and otherwise a position number."
  ...
  (let* ((menu 	(imenu--split-menu (imenu--sort-list index-alist)
	 ...
	 (or (equal index-alist imenu--last-menubar-index-alist)
	     (let (menu menu1 old)
	       ...
	       (setq index-alist (imenu--split-submenus
				  (imenu--sort-list index-alist)))
	       (setq menu (imenu--split-menu index-alist (buffer-name)))
	       (setq menu1 (imenu--create-keymap-1
			    (car menu) 
			    (if (< 1 (length (cdr menu)))
				(cdr menu)
			      (cdr (car (cdr menu))))
