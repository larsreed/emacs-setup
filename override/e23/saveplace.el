...
\(rather than an absolute offset from the beginning of the file\)."
  :group 'save-place
  :type 'boolean)

(defconst save-place-eof-marker -1
  "A special value indicating end-of-file")
  ...
  (if buffer-file-name
      (save-match-data
        (let ((cell (assoc buffer-file-name save-place-alist))
	      (position (if (and save-place-keep-eof
				 (= (point) (point-max)))
			    save-place-eof-marker
	  ...
	  (if (and save-place
		   ;; Optimize out the degenerate case.
		   (not (= position 1))
		   ;; And check for ignored buffers
		   (not (string-match
			 (concat "\\("
