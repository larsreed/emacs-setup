...
(defcustom recentf-always-list nil
  "* List of files that should always be on the menu"
  :group 'recentf
  :type '(repeat file))

(defcustom recentf-version-control nil
  "*Controls whether to make numbered backups `recentf-save-file'.
...
list of recently opened files."
  :group 'recentf
  :type '(radio (function-item recentf-find-file)
		(function-item find-file)
		(function-item recentf-find-file-other-window)
		(function-item find-file-other-window)
		(function-item recentf-find-file-other-frame)
		(function-item find-file-other-frame)
		function))

;; Should have been per submenu - not grand total!
(defcustom recentf-max-menu-items 10
  "Maximum number of items in the recentf menu."
  :group 'recentf
  :type 'integer)

(defcustom recentf-max-submenu-items 10
  "*Maximum number of items in a submenu."
  :group 'recentf
  :type 'integer
  :set 'recentf-menu-customization-changed)
...
;;; Utilities
;;
;;   Why is there no built-in `filename-ignore-case' to avoid these hacks???
...
(defun recentf-find-file-common (filename func)
  "Edit file FILENAME using FUNC.
...
If FILENAME is not readable it is removed from `recentf-list'."
  (recentf-find-file-common filename 'find-file))

(defun recentf-find-file-other-window (filename)
  "Edit file FILENAME using `find-file-other-window'.
If FILENAME is not readable it is removed from `recentf-list'."
  (recentf-find-file-common filename 'find-file-other-window))

(defun recentf-find-file-other-frame (filename)
  "Edit file FILENAME using `find-file-other-frame'.
Unless the current frame only contains an empty *scratch*-buffer, in which
case `find-file' is used.
If FILENAME is not readable it is removed from `recentf-list'."
  (if (and (one-window-p t) (buffer-size)
           (string= "*scratch*" (buffer-name))
           (= (buffer-size) 0))
      (recentf-find-file-common filename 'find-file)
    (recentf-find-file-common filename 'find-file-other-frame)))
...
menu-elements (no sub-menu)."
  (if (and l (functionp filter))
      (let ((case-fold-search recentf-case-fold-search)
        ...
        (when others
          (setq others (recentf-trunc-list
                        (funcall filter (nreverse others))
                        recentf-max-submenu-items)))
...
;; Should have had a uniquify function to reduce string lengths!
...
optional argument NO-DIR is non-nil, or its directory otherwise."
  (let (filtered-names filtered-list full name counters sufx name-count)
    (dolist (elt l)
      (let* ((name (file-name-nondirectory (recentf-menu-element-value elt)))
             (has-it (assoc name name-count)))
        (if has-it
            (setcdr has-it (1+ (cdr has-it)))
          (push (cons name 1) name-count))))
    (dolist (elt l (nreverse filtered-list))
      (setq full (recentf-menu-element-value elt)
            name (file-name-nondirectory full))
      (if (= 1 (cdr (assoc name name-count))) ;;(not (member name filtered-names))
...
use for the dialog.  It defaults to \"*`recentf-menu-title'*\"."
  ...
  (recentf-dialog (or buffer-name (format "*%s*" recentf-menu-title))
    ...
    (if (fboundp 'tree-widget-set-theme)
...
Write data into the file specified by `recentf-save-file'."
  ...
  (condition-case error
      (with-temp-buffer
        (let ((version-control
               (cond
                ((null recentf-version-control) nil)
                ((eq 'never recentf-version-control) 'never)
                ((eq 'nospecial recentf-version-control) version-control)
                (t t))))
          (erase-buffer)
    ...
    (when (file-readable-p file)
      ...
      (if recentf-always-list
          (let ((recentf-exclude nil))
            (mapcar 'recentf-add-file recentf-always-list)))
